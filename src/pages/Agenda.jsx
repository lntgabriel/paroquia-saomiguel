...
const Agenda = () => {
  // ATENÇÃO: Mudei para iniciar em 2026
  ...
  // --- LÓGICA INTELIGENTE DE EVENTOS FIXOS (COMUNIDADES) ---
  const getEventosCompletos = (dia) => {
    const dataObj = new Date(ano, mes, dia);
    const diaSemana = dataObj.getDay(); // 0 = Dom, 6 = Sab
    
    // Formata YYYY-MM-DD para buscar no banco de eventos especiais
    const dataString = `${ano}-${String(mes + 1).padStart(2, '0')}-${String(dia).padStart(2, '0')}`;
    
    // 1. Pega os eventos especiais do arquivo
    let listaEventos = eventosDB.filter(e => e.data === dataString);

    // 2. Adiciona os horários FIXOS das comunidades baseados no dia da semana
    
    // DOMINGO (0)
    if (diaSemana === 0) {
        listaEventos.push({ titulo: 'Missa', hora: '08:00', local: 'Matriz São Miguel', tipo: 'fixo' });
        listaEventos.push({ titulo: 'Missa', hora: '10:00', local: 'Matriz São Miguel', tipo: 'fixo' });
        listaEventos.push({ titulo: 'Missa', hora: '10:00', local: 'Com. Imaculada', tipo: 'fixo' });
        listaEventos.push({ titulo: 'Missa', hora: '18:00', local: 'Com. Santa Rita', tipo: 'fixo' });
        listaEventos.push({ titulo: 'Missa', hora: '18:00', local: 'Com. Santa Edwiges', tipo: 'fixo' });
        listaEventos.push({ titulo: 'Missa', hora: '19:30', local: 'Com. Natividade', tipo: 'fixo' });
    }
    ...
    if (diaSemana === 5) {
        ...
        // Verifica se é a Primeira Sexta do Mês (Sagrado)
        if (dia <= 7 && diaSemana === 5) { 
             listaEventos.push({ titulo: 'Missa do Sagrado Coração', hora: '20:00', local: 'Com. Sagrado', tipo: 'importante' });
        }
    }

    // SÁBADO (6)
    if (diaSemana === 6) {
        listaEventos.push({ titulo: 'Missa Vigília', hora: '18:00', local: 'Com. Divino Espírito Santo', tipo: 'fixo' });
        listaEventos.push({ titulo: 'Missa Vigília', hora: '19:30', local: 'Com. Sagrado', tipo: 'fixo' });
        listaEventos.push({ titulo: 'Missa Vigília', hora: '20:00', local: 'Com. NS do Carmo', tipo: 'fixo' });
    }

    // DIA 29 (São Miguel - Já está no eventos.js, mas garanto aqui caso queira fixo)
    if (dia === 29 && diaSemana !== 0) { // Se cair domingo ja tem horario normal, se n, é 20h
        // Já incluso no eventosDB geralmente, mas ok deixar redundante se quiser
    }

    return listaEventos;
  };

  // Função auxiliar para mostrar apenas bolinhas/resumos no card
  const getTiposEventosDia = (dia) => {
      const evts = getEventosCompletos(dia);
      return evts.map(e => e.tipo); // retorna array ['fixo', 'festa', etc]
  };
  ...
  const getCorTag = (tipo) => {
      ...
      if(tipo === 'fixo') return 'text-gray-600 bg-gray-50 border-gray-100';
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center md:items-end mb-6 md:mb-8 text-center md:text-left">
          <div className="mb-4 md:mb-0">
            <span className="text-parish-terracotta font-bold tracking-[0.2em] text-[10px] md:text-xs uppercase mb-1 block">Calendário 2026</span>
          ...
          <div className="hidden md:flex items-center gap-2 text-sm text-gray-500 bg-white px-4 py-2 rounded-full shadow-sm border border-gray-200">
             ...
             <span className="font-bold text-parish-dark">Ano Litúrgico {ano}</span>
          </div>
        </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex flex-wrap justify-center md:justify-start gap-1 mt-1 md:mt-2">
                     {tiposEventos.slice(0, 4).map((tipo, idx) => (
                        <div key={idx} className={`w-1.5 h-1.5 md:w-2 md:h-2 rounded-full ${getCorBolinha(tipo)}`}></div>
                     ))}
                     {tiposEventos.length > 4 && <span className="text-[8px] text-gray-400 leading-none">+</span>}
                  </div>
                </div>
              );
            })}
        ...
        {/* DETALHES (Mobile & Desktop Integrado em baixo) */}
            ...
            {diaSelecionado ? (
                <div className="bg-white p-6 md:p-8 rounded-2xl shadow-lg border border-parish-gold/20 animate-slideUp">
                    <div className="flex items-center gap-3 mb-6 border-b border-gray-100 pb-4">
                        ...
                        <div>
                            <h3 className="font-serif font-bold text-2xl text-parish-dark">Programação do Dia</h3>
                    ...
                    <div className="grid md:grid-cols-2 gap-4">
                        {eventosDoDiaSelecionado.length > 0 ? (
                                ...
                                <div key={idx} className={`p-4 rounded-xl border flex items-start gap-4 ${getCorTag(evt.tipo).replace('text-','border-')}`}>
                                    ...
                                    <div>
                                        ...
                                        <div className="flex gap-3 text-sm text-gray-600">
                                            {evt.hora && <span className="font-mono bg-gray-100 px-2 rounded font-bold">{evt.hora}</span>}
                            ...
                            <p className="text-gray-400 italic col-span-2 text-center py-4">Nenhuma celebração oficial cadastrada para hoje.</p>
                        )}
                    </div>
                </div>
            ) : (
                <div className="text-center py-8 text-gray-400 border-t border-dashed border-gray-300">
                    ...
                    <p>Toque em um dia no calendário para ver os horários das missas nas comunidades.</p>
                </div>
            )}
        </div>
